...
class ClientServersTest(unittest.TestCase):
  ...
  def test_persistence(self):
    ...
    server = ServerServicer(out_file="test_out")

    # Create a server and create two users
    auth_a = server.Authenticate(service_pb2.AuthenticateRequest(
      username="test_a",
      password="test"
    ), None)
    ...
    # Create new server; test if those two users are still here
    server_new = ServerServicer(out_file="test_out")
    response = server_new.List(service_pb2.ListRequest(
      token=auth_a.response,
      request="\n"
    ), None)
    self.assertTrue(response.success)
